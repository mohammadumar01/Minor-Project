...
def parse_fattree_shape(shape_str):
    ...
    #for fattree shape "2,2:2" downlinks = [2, 2]
    pods = downlinks[-1]           # 2 pods
    hosts_per_pod = downlinks[0]   # 2 hosts per pod
    return pods, hosts_per_pod, downlinks

def fattree_hops(src, dst, downlinks):
    """Compute hop count in FatTree topology"""
    hosts_per_pod = downlinks[0]
    ...
    if pod_src == pod_dst:
        ...
        return 5      # Different pods

# Parse FatTree shape
shape_str = "2,2:2"
pods, hosts_per_pod, downlinks = parse_fattree_shape(shape_str)
total_hosts = pods * hosts_per_pod  # 4 hosts
...
motifs = [
    ...
    {"pattern": "PingPong", "pairs": [(0,1), (0,2), (0,3)], "iterations": 105}
]
